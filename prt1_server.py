...
def is_available(file_name):
    ...
    return file_name in file_list and os.path.exists(FILE_LIST_PATH)

def create_protocol(method, file_name):
    message = f"{method}{DELIMITER}{file_name}"
        ...
        if len(msg_parts) == 2:
            if method == "GET":
                if is_available(file_name):
                    ...
                    client.send(create_protocol("ERR", file_name))
